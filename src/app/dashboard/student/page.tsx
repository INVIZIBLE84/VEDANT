...
import { getFeeDetails } from "@/services/fee-management";
import { getStudentClearanceStatus, calculateProgress as calculateClearanceProgress } from "@/services/clearance";
...
export default function StudentDashboardPage() {
    ...
    const [feeDetails, setFeeDetails] = React.useState<any>(null);
    const [clearanceDetails, setClearanceDetails] = React.useState<any>(null);
    ...
    if (error) {
     ...
     if (!user) {
        // Should ideally redirect to login, but show message for now
        return (
             <div className="space-y-6">
                <h1 className="text-3xl font-bold text-primary">Student Dashboard</h1>
                <Alert variant="destructive">
    ...
    const attendanceStyle = getStatusStyle(todayAttendance?.status || 'Not Marked');
    const feeStyle = getStatusStyle(feeDetails?.status || 'Unpaid');
    const clearanceProgress = clearanceDetails ? calculateClearanceProgress(clearanceDetails.steps) : 0;
     const clearanceStyle = getStatusStyle(clearanceDetails?.overallStatus || 'Pending');
                ...
                <Link href="/attendance">
                     <Card className="group transform transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <CardHeader>
                            ...
                            <CardDescription>Today: <Badge variant={attendanceStyle.variant} className={cn("ml-1", attendanceStyle.className)}>{todayAttendance?.status || 'Not Marked'}</Badge></CardDescription>
                        </CardHeader>
                        <CardContent>
                             {attendanceSummary ? (
                                <>
                    ...
                    <Card className="group transform transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><DollarSign className="text-secondary" /> Fee Status</CardTitle>
                             <CardDescription>Current Status: <Badge variant={feeStyle.variant} className={cn("ml-1", feeStyle.className)}>{feeDetails?.status || 'Unavailable'}</Badge></CardDescription>
                        </CardHeader>
                        <CardContent>
                            {feeDetails ? (
                                <>
                    ...
                    <Card className="group transform transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2"><CheckCircle className="text-accent" /> Clearance</CardTitle>
                             <CardDescription>Overall Status: <Badge variant={clearanceStyle.variant} className={cn("ml-1", clearanceStyle.className)}>{clearanceDetails?.overallStatus || 'Not Submitted'}</Badge></CardDescription>
                        </CardHeader>
                        <CardContent>
                             {clearanceDetails ? (
                                <>
                                ...
                                <Button size="sm" variant="outline" className="w-full" disabled>Submit Clearance Request</Button> // Link/enable when implemented
                            )}
                        </CardContent>
                    </Card>
                </Link>
